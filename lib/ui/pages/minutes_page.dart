...
class MinutesPage extends StatefulWidget {
  const MinutesPage({Key key}) : super(key: key);
...
class _MinutesPageState extends State<MinutesPage> {
  ...
  List<MinutesModel> _listModel = [];

  @override
  void initState() {
    changeStatusBar(mainColors[selectU], true);
    _listModel = context.read<UssdProvider>().listMinutes;
    super.initState();
  }
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(lang ? 'Минуты' : 'Daqiqalar', style: kTextStyle(fontWeight: FontWeight.w600),),
          centerTitle: true,
        ),
        body: TabBarView(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 8),
                          child: Text(lang ? 'Количество минут:' : 'Daqiqa hajmi:', style: kTextStyle(color: textGreyDark, size: 12, fontWeight: FontWeight.w500),),
                        ),
                      ),
